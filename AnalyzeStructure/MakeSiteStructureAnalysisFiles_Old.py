...
def get_read_structural_data(read_seqs, _mirna, _sitelist, site, experiment,
    ...
    col_names = ["flank", "plfold", "accessibility", "AU_cs", "AU_win", "AU_read",
    ...
    for i_r, r in enumerate(read_seqs):
        ...
        if _read.sites[0].name != "None":
            ...
            if site in read_sites:
                ...
                row = pd.DataFrame([flank, p_acc_pl, p_acc_geo, AU_cs] +
                                   [float(sum(i))/len(i) for i in [
                    AU_win, AU_read, AU_win_wo, AU_read_wo]], index=col_names).T
...
def main():
    ...
    arguments = ["miRNA", "experiment", "condition", "n_constant", "sitelist",
                 "site", "win_start", "win_stop", "-buffer3p_binary", "-jobs",
                 "-test_binary"]
    (
    ...
    #              in output_matrix[flank]
    #          ]) for flank in flanks if len(output_matrix[flank]) > 0])
