...
def get_average_structure(read_seqs,order_site_map,win_size,ind_start=0, ind_stop=0,constant=False):
    ...
    win_pos_rel = sorted(set([i - j for i, j in it.product(win_pos,sites_range)]))
    print(win_pos_rel)
    ...
    for count,seq in enumerate(read_seqs):
        ...
        if sites != "None":
            ...
            if start in sites_range and stop in sites_range and site == "8mer":
                print(read)
                print(site)
                print(coord)
                print(" "*min(seq_range)+read[min(seq_range):max(seq_range)+1])
                print(" "*start+read[start:stop])
                prob_pos = [float(j) for j in structure.split("\t")]
                prob_win_pos = [sum(prob_pos[i:i+win_size])/win_size for i in range(len(read)-win_size+1)]
                for i in range(len(read)):
                    if i in seq_range:
                        rel_pos = win_pos[i]-start
                        print("."*i+str(rel_pos))
                        print(" "*start+read[start:stop])
                        for j in range(win_size):
                            print(" "*(i+j)+str(prob_pos[i+j]))
                        print("_"*i+str(prob_win_pos[i]))
...
def main():
    ...
    for win_size in range(1,21):
        site_averages_path = get_analysis_path(mirna,experiment,condition,"structures_bp_prob",ext="_%s-%s_average_window_%s_test" %(ind_start,ind_stop,win_size))
        site_totals_path = get_analysis_path(mirna,experiment,condition,"structures_bp_prob",ext="_%s-%s_totals_window_%s_test" %(ind_start,ind_stop,win_size))
