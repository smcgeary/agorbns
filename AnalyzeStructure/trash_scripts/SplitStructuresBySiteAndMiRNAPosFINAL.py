...
def pull_structures(read_seqs, order_site_map, ind_start, ind_stop,
                    win_start, win_stop, constant = True, mir_center = False):
...
def main():
    ...
    arguments = ["miRNA", "experient", "condition", "ind_start", "ind_stop", "sitelist", "win_start", "win_stop", "constant", "mir_win"]
    mirna, experiment, condition, ind_start, ind_stop, sitelist, win_start, win_stop, constant, mir_win = parse_arguments(arguments)
    ...
    print(output_structures)
    print(output_AU_win)
    print(output_AU_read)
    summary = {site: {kmer: "0" for kmer in kmers} for site in order_site_map.keys() if site != "None"}
    file_ext = "_%s-%s_%s_%s-%s" %(ind_start,ind_stop,sitelist,win_start,win_stop)
    if constant:
        file_ext = file_ext + "_plusconstant"
    else:
        file_ext = file_ext + "_minusconstant"
    if mir_win == False:
        file_ext = file_ext + "_relmiRNA"
    else:
        file_ext = file_ext + "_reltarget"
