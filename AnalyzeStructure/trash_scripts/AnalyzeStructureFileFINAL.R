...
final_round <- final_mat - min(final_mat)
final_round_round <- final_round / max(final_round)
for (j in seq(20)){
	x_tot <- c()
	y_tot <- c()
for (i in seq(-17,20)) {
...
# 		# })
	...
	# points(x_now,y_new,col=rainbow(20)[j],pch=19)
				 rect(x_now-0.5,j-0.5,x_now+0.5,j+0.5,col=rainbow(100, start = 0.05, end = 0.7)[100-round(final_round_round[j, i + 18]*99)],
				 border=NA)
