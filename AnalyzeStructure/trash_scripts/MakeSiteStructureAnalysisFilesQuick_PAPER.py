...
def get_read_structural_data(read_seqs, order_site_map, site_pos_map, n_constant,
    ...
    for num_i, i in enumerate(read_seqs):
        ...
        if sites != "None":
            ...
            if start in sites_range and stop in sites_range and site in ["8mer", "7mer-m8", "7mer-A1", "6mer", "6mer-A1", "6mer-m8"]:
                ...
                if constant == False:
                    win_l = max(win_l,26 - n_constant)
                    win_r = min(win_r,26 + 37 + n_constant)
                AUleftoffset = 30 - len(AU_flanks_left)
                AUrightoffset = len(AU_flanks_right)
                AU_context_score = get_local_au_score(read, start, stop, site)
                AU_win = float((read[win_l:win_r].count("A") + read[win_l:win_r].count("T")))/(win_r - win_l)
                AU_read = float((read[26 : 26 + 37].count("A") + read[26 : 26 + 37].count("T")))/37
