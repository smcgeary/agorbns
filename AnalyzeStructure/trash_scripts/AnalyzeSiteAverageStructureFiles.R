...
col_ind <- function(x) {
				 	if (is.na(x)) {
			 			...
			 			return(round(min(max((x+0.25)/1.5*(150)+70,1),100)))
				 	}
				}
...
# 	"/average_structure_enrichments/",mirna,"/",site,".pdf"),width=20,height=15)
	...
	sapply(seq(1,20),function(win_size) {
		print(win_size)
			window_I <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
		                  ...
		                  "_0-0_average_window_",win_size,".txt")
			total_I <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
		                  "/equilibrium/structures_bp_prob/","I",
		                  "_0-0_totals_window_",win_size,".txt")
			...
			print(x_dat[inds])
			print(x_dat)
			data = log((probs_AGO[,site])/(probs_I[,site]),base=2)
			print("data")
			print(data[inds])
