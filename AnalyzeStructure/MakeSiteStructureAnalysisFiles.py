################################################################################
#MakeSiteStructureAnalysisFiles.py
################################################################################
...
def RNAplfold(read, mir_start, mir_stop, file_num, outputdir=""):
    """
    ...
    call = 'RNAplfold -L %s -W %s -u %s < %s%s.fa' %(
        l_param, w_param, u_param, read_name, file_num
    )
    subprocess.call([call], shell=True, stdout = PIPE)
    ...
    for i_r, r in enumerate(read_seqs):
        ...
        if _read.sites[0].name != "None":
            ...
            if site in read_sites:
                ...
                if "b" in site:
                    win_l -= 1
                plfold = RNAplfold(_read.fullseq, win_start, win_stop, num_i, outputdir=outputdir)
...
def main():
    ...
    plfold_path = "AnalyzeStructure/RNAplfold_temp/%s" %folder_name
    print(plfold_path)
    ...
    args  = [_mirna, _sitelist, site, experiment, int(n_constant), read_length, buffer3p,
             int(win_start), int(win_stop), plfold_path + "/"]
