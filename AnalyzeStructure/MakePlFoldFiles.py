...
def RNAplfold(read, win, mir1p, file_num, logprob=False):
    ...
    # print("before row part")
    # sys.stdout.flush()
    # print(rnaplfold_data)
    # sys.stdout.flush()
    for row in range(rnaplfold_data.shape[0]):
        # print(row)
        # sys.stdout.flush()
        # print(rnaplfold_data.iloc[row, ])
        # sys.stdout.flush()
        # print(-((row + 1)/2))
        # sys.stdout.flush()
        # print((1/float(row + 1)))
        # sys.stdout.flush()
        shift_ind = -((row + 1)//2)
        exponent = 1/float(row + 1)
        shifted_row = (rnaplfold_data.iloc[row, ].shift(shift_ind))**exponent
        rnaplfold_data.iloc[row, ] = shifted_row
    # print(rnaplfold_data)
    # sys.stdout.flush()
...
def main():
    ...
    outputmatrix_old = pd.DataFrame(0.0, index=df_final_index, columns=df_final_columns)

    for flank in flanks:
        flank_matrix = pd.Series(plmean_flanks_map_final[flank].values.flatten())
        flank_matrix.index = df_final_index
        outputmatrix_old.loc[df_final_index, flank] = flank_matrix

    outputmatrix = pd.DataFrame.from_dict({
        flank : plmean_flanks_map_final[flank].values.flatten()
        for flank in flanks})
    outputmatrix.index = df_final_index

    print(outputmatrix_old)
    print(outputmatrix)
    ...
    output_path = get_analysis_path(
        mirna, experiment, condition, "plfold_windows_by_flanks", ext=extension)
    print(output_path)
