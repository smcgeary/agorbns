...
from repression_functions import get_kathy_mirna_name
...
def train_on_data(train_vals, num_feats, passenger, outfile, init_bound, set_vars={}):
    ...
    with tf.Session() as sess:
        ...
        mod.fit(sess, train_data, train_feed_dict, maxiter=300)
...
if __name__ == '__main__':
    ...
    ## SEAN EDITING THE CODE HERE
    parser.add_option("--train_mir", dest="TRAIN_MIR", help="fit to only a single miRNA", default=None)
    ...
    #### SEAN CODE TO TRAIN ONLY A SINGLE MIRNA
    if options.TRAIN_MIR is not None:
        TRAIN_GUIDES = [get_kathy_mirna_name(options.TRAIN_MIR)]
    print(TRAIN_GUIDES)

    print(f'Number training guides: {len(TRAIN_GUIDES)}')
    print(options.PASSENGER)
    ...
    print(TRAIN_MIRS)
    # read in TPM data
    print(options.TPM_FILE)
    ALL_TPMS = pd.read_csv(options.TPM_FILE, sep='\t', index_col=0)
    ## SEAN ADDED THIS BECAUSE THIS TRANSCRIPT IS NOT IN ANY OF KATHY'S ########
    ## OUTPUT FILES. ###########################################################
    ALL_TPMS.drop(index="NM_006772.2", inplace=True)
    print(ALL_TPMS.iloc[:10, :10])
    ## SEAN ADDED THIS IN IN ORDER TO CONVERT TO KATHY'S NAMING CONVENTION #####
    ALL_TPMS.columns = [i.replace("R", "r").replace("-", "").replace("let7a", "let7") for i in ALL_TPMS.columns]
    ...
    for mir in TRAIN_MIRS:
        ...
        if mir.replace('_pass', '') == options.MIR_TO_SHUFFLE:
            ...
            # Here the path of the file is constructed in order to input the 
            # feature file.
            feature_file_path = options.FEATURE_FILE.replace('MIR', mir)
            if mir in ["mir7", "lsy6", "mir124"]:
                feature_file_path = feature_file_path.replace("programmed", "random")
            print(feature_file_path)
            temp = pd.read_csv(feature_file_path, sep='\t')
        temp['mir'] = mir
    ...
    time_stop = time.time()
    print(("Finished job future in %3.2f seconds." %(time_stop - time_start)))
