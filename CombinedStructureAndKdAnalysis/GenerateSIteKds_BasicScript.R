...
pars.init <- c(Logit(kds.init, max = 1), -1, -1, -1, -1, -1, log10(k.c.stockago))
trial <- pars.init
# trial[trial == Inf] <- 0
# trial[is.na(trial)] <- 0
# trial[trial == -1*Inf] <- 0
...
trial <- pars_new$par
MakeSiteIterationPlot(out,paste0(experiment,"_", start, "-", stop, "_", win_left, "-", win_right,"_Basic"),mirna, num.kds, num.bgs)  

    out<<- rbind(out, c(trial, -1*prob_pois))
          out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
                   "/", experiment, "/kds_with_structure/",
                   start, "-", stop, "_", win_left, "-", win_right, "_Basic.txt")
