  ...
  if (tick%%100 == 0) {
    ...
    # dev.off()
      out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
                   "/", experiment, "/kds_with_structure/", k.c.stockago, "_",
                   start, "-", stop, "_", win_left, "-", win_right, "_preliminary_dinucs_single.txt")
...
out <- read.table(file = paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/",
           ...
           "_-3--3_1-15_preliminary_dinucs.txt"))
...
start <- GetModelFrequencies(trial, out)
    temp <- trial
for (i in seq(1,100)) {
  # scale <- sapply(seq(1,length(trial)), function(par_i) {

  #   temp[par_i] <- 1.1*temp[par_i]
  #   func_out <- abs(GetModelFrequencies(temp,out) - start)/(0.1*temp[par_i]))
  #   return(func_out)
  ...
  pars_new <- optim(trial,
                    ...
                    out_model = out,
                    method = "Nelder-Mead",
                    control = list(maxit = 5000, alpha = 1, beta = 0.5, gamma = 2, parscale = scale))
print("ADD")
