...
# Modeling Functions
#___________
GetOccupancy <- function(c.freeago, kds, ps, p.exponent) {
  ...
  return(c.freeago * (c.freeago + kds * ps^(-p.exponent))^(-1))
}

#______________
GetFreeResidual <- function(c.freeago, kds, ps, c.tots, p.exponent,
                            c.ago) {
  occs_I <- GetOccupancy(c.freeago, kds, ps, p.exponent)
  residual <- (c.ago - c.freeago - sum(occs_I*c.tots))^2
  return(residual)
}

#_________
GetBoundRNA <- function(kds, ps, c.tots, p.exponent, c.ago) {
  c.free <- optimize(GetFreeResidual,
                     ...
                     c.ago = c.ago)$minimum
  c.bound <- GetOccupancy(c.free, kds, ps, p.exponent)*c.tots
  return(c.bound)
}
...
c.I.tots <- c(I.data$n,sitesXcounts["None", 1])
c.I.tots <- c.I.tots / sum(c.I.tots) * 100
...
r_numerator <- sitesXcounts[, 3] / sum(sitesXcounts[, 3])
r_denomenator <- sitesXcounts["I"] / sum(sitesXcounts["I"])
kds.init <- (r_numerator / r_denomenator)[1 : num.kds, 1]
# pars.init <- c(-2*log10(kds.init), -1, -1, -1, -1, -1, 0.6, 0)
pars.init <- GetSiteKds(mirna, k.c.stockago, start, stop, win_left, win_right)
pars.init <- as.numeric(pars.init[-nrow(pars.init),ncol(pars.init)])
print(pars.init)
trial <- pars.init
tick <- 0
# Define function of just kds and pars.
c.agos <- sapply(colnames(data), function(x) {
   as.numeric(x) / 100 * k.c.stockago*0.6
  })

c.totals <- matrix(rep(c.I.tots, times = 5),ncol=5)

GetModelFrequencies <- function(pars, c.I.model = c.I.tots, probabilities.c = p.I, I.IDs.model = I.IDs, out_total = out) {
  ...
  probabilities.c <- c(probabilities.c, 1 / (1 + exp(-bg.a)))
  # print('hi')
  ...
  c.bounds <- as.matrix(
    sapply(c.agos, function(x) {
             return(GetBoundRNA(kds, probs, c.I.model, k, x))
           }
           ))
  # print(dim(c.bounds))
  # print(dim(c.totals))
  ...
  if (tick%%10 == 0) {
    ...
    out_new <- rbind(out_total, c(pars, prob_pois))
    out <<- out_new
      MakeSiteIterationPlot(out_new,paste0("structure", start, "-", stop, "_", win_left, "-", win_right,"_probs"))  

    # setEPS()
    # postscript(file=paste0("/lab/bartel1_ata/mcgeary/computation/AgoRBNS/figures/kds/",
    # mirna,"/", "structure_fit_", start, "-", stop, "_", win_left, "-", win_right, "_stoch.eps"))
    plot(c(0.0001,0.0002),c(0.0001,0.0002),col="white",xlim=c(0.0000002,.02),ylim=c(0.0000002,.02), log='xy')
...
names(pars.init) <- c(rownames(sitesXcounts)[1:num.kds],colnames(data),"unpaired_k","bg_alpha")
...
wiggle <- 1
# while (i < 10000) {
#   rand_ind <- sample(seq(length(pars.init)), 1)
#   # print(rand_ind)
#   pars_current <- out[nrow(out), -ncol(out)]
#   val <- out[nrow(out), ncol(out)]
#   par <- pars_current[rand_ind]
#   # print("par")
#   # print(par)
#   del <- rnorm(1,0,wiggle)
#   par_new <- c(par + del, par - del)
#   # print("par_new")
#   # print(par_new)
#   val_new <- sapply(par_new, function(par_i) {
#     GetModelFrequencies(par_i, rand_ind, pars_current)
#   })
#   best <- which(val_new == min(val_new))
#   if (val_new[best] < c(val)) {
#     print(val_new[best] < val)
#     print(val_new[best])
#     pars_current[rand_ind] <- par_new[best]

#     out <- rbind(out,c(pars_current,val_new[best]))

#     out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
#                      "/equilibrium/kds_with_structure/", k.c.stockago, "_",
#                        start, "-", stop, "_", win_left, "-", win_right, "_probs.txt")
#     # write.table(file=out_file, out, sep="\t", quote=FALSE, row.names=FALSE,
#     #             col.names=TRUE)

#     MakeSiteIterationPlot(out,paste0("structure", start, "-", stop, "_", win_left, "-", win_right,"_probs"))  
#     i <- i + 1
#   } else {
#     wiggle <- 0.99 * wiggle
#     print("stochastic:")
...
solution <- optim(pars.init,GetModelFrequencies,c.I.model = c.I.tots, probabilities.c = p.I, I.IDs.model = I.IDs,out_total = out, control = list(maxit=200000))
