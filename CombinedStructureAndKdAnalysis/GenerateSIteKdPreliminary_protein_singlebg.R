...
remove <- which(nchar(site_seqs) > 9)
...
pars.init <- c(-log10(kds.init) - 1, -1, log10(k.c.stockago))
print('hi')
trial <- pars.init
trial[trial == Inf] <- 0
trial[is.na(trial)] <- 0
...
GetModelFrequencies <- function(pars, out_model) {
  ...
  if (tick%%1000 == 0) {
    ...
    print(-1*prob_pois)
    print(colSums(c.bgs) / colSums(c.final))
    # setEPS()
    ...
    # dev.off()
      out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
                   ...
                   start, "-", stop, "_", win_left, "-", win_right, "_preliminary_protein.txt")
      write.table(file=out_file, out, sep="\t", quote=FALSE, row.names=FALSE)

    MakeSiteIterationPlot(out,paste0(experiment, start, "-", stop, "_", win_left, "-", win_right,"_preliminary_protein"),mirna)  
