...
pars.init <- c(Logit(kds.init, max = 1), -1, -1, -1, -1, -1)
names(pars.init) <- c(rownames(data), colnames(data))
# trial <- pars.init
# trial[trial == Inf] <- 0
# trial[is.na(trial)] <- 0
# trial[trial == -1*Inf] <- 0
...
GetModelFrequencies <- function(pars, out_model) {
  ...
  if (tick%%1000 == 0) {
    ...
    MakeSiteIterationPlot(out,paste0(experiment,"_", start, "-", stop, "_", win_left, "-", win_right,"_Basic_fixed_protein"),mirna, num.kds, num.bgs)  
...
trial <- pars_new$par
    out<<- rbind(out, c(trial, -1*prob_pois))
          out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
                   ...
                   start, "-", stop, "_", win_left, "-", win_right, "_Basic_fixed_protein.txt")
