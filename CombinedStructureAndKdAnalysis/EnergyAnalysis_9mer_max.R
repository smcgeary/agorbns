...
GetKdTable <- function(mirna,experiment) {
	...
	table <- read.table(file = paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/",
		       ...
		       "-3--3_1-15_preliminary_protein.txt"))
	table <- t(table)
...
cols = c(site_cols[rownames(kds_nb),][1:243],rep("gray",5))
ago_ind <- which(rownames(kds_sm) == "AGO")
cols[ago_ind] <- "darkslateblue"
...
plot(seq(1,min(ncol(kds_nb),ncol(kds_sm)), length = 50),
	 sapply(seq(1,min(ncol(kds_nb),ncol(kds_sm)), length = 50),
	 	    function(i){cor(as.numeric(kds_sm[,ncol(kds_sm)-min(ncol(kds_nb),ncol(kds_sm)) + i]),as.numeric(kds_nb[,ncol(kds_nb)-min(ncol(kds_nb),ncol(kds_sm)) + i]))}),
	 ann = FALSE)
...
plot(seq(1,min(ncol(kds_nb_seed),ncol(kds_sm)), length = 50),
	 sapply(seq(1,min(ncol(kds_nb_seed),ncol(kds_sm)), length = 50),
	 	    function(i){cor(as.numeric(kds_sm[,ncol(kds_sm)-min(ncol(kds_nb_seed),ncol(kds_sm)) + i]),as.numeric(kds_nb_seed[,ncol(kds_nb_seed)-min(ncol(kds_nb_seed),ncol(kds_sm)) + i]))}),
	 ann = FALSE)
...
plot(seq(1,min(ncol(kds_nb_seed),ncol(kds_nb)), length = 50),
	 sapply(seq(1,min(ncol(kds_nb_seed),ncol(kds_nb)), length = 50),
	 	    function(i){cor(as.numeric(kds_nb_seed[,ncol(kds_nb_seed)-min(ncol(kds_nb_seed),ncol(kds_nb)) + i]),as.numeric(kds_nb[,ncol(kds_nb)-min(ncol(kds_nb_seed),ncol(kds_nb)) + i]))}),
	 ann = FALSE)
...
dev.new(xpos = 50, ypos = 500, width = 16, height = 5)
par(mfrow=c(1,4))
