...
get_trinuc_indeces <- function(word) {
...
get_prob_from_weights <- function(weights,probs_freqs) {
  ...
  # print(ncol(weights))
  return(exp(sum(log(base_prob)))*(37 + as.integer(start) + as.integer(stop) - ncol(weights) - 2 + 1))
}
print(get_prob_from_weights(get_trinuc_indeces("ACAC"),rep(1,64)))
...
data <- sitesXcounts[,c(2,8)]
site_seqs <- as.character(sitesXcounts[,1])
site_weights <- lapply(site_seqs[1:num.kds],get_trinuc_indeces)
c.I.tots <- matrix(unlist(rep(sitesXcounts["I"]/sum(sitesXcounts["I"])*k.c.lib,
...
GetModelFrequencies <- function(pars, out_model) {
  ...
  weights <- c(10^pars, 1)
  freqs <- c(sapply(site_weights,get_prob_from_weights,probs_freqs = weights))
  for (i in seq(1,length(freqs))) {
    freqs[i] <- 2*freqs[i] - sum(freqs[grep(site_seqs[i],site_seqs)])
  }
  freqs <- c(freqs, 1 - sum(freqs))
  sapply(1:length(c.agos), function(col) {
    c.totals[,col] <<- freqs*k.c.lib
    })
  c.final <- data.frame(t(t(c.totals) / colSums(c.totals)))
...
# print(out)
colnames(out) <- c(trinucs[1:(length(trinucs) - 1)],"-logp")
# print(out)
