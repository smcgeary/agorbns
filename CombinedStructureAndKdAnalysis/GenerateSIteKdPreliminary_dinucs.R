  ...
  if (tick%%100 == 0) {
    ...
    # dev.off()
      out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
                   ...
                   start, "-", stop, "_", win_left, "-", win_right, "_preliminary_dinucs.txt")
      write.table(file=out_file, out, sep="\t", quote=FALSE, row.names=FALSE)

    MakeSiteIterationPlot(out,paste0(experiment,"_", start, "-", stop, "_", win_left, "-", win_right,"_preliminary_dinucs"),mirna)  
