...
trinucs <- apply(expand.grid(c("A", "C", "G", "T"),c("A","C","G","T"),c("A","C","G","T")),
                 1, function(row) {
                 paste(rev(row),collapse = "")})

GetNucleotideProbIndeces <- function(word) {
  letters <- nchar(word) - 3 + 1
  den <- 1:length(trinucs)
  sapply(1:letters, function(i) {
    tri <- substr(word,i,i + 2)
    num <- which(trinucs == tri)
    out <- list(num, den)
    den<<- which(substr(trinucs,1,2) == substr(tri,2,3))
    return(out)
    })
}
...
sitesXcounts <- sitesXcounts[c("8mer","7mer-m8", "7mer-A1", "6mer","6mer-m8","6mer-A1",
                              "5mer-A1", "5mer-m2.6", "5mer-m3.7", "5mer-m8", nrow(sitesXcounts)),]
...
double.windows_pre <- outer(nchar(site_seqs),nchar(site_seqs),FUN="+")
GetDoubleWindows <- function(sum) {
  return(2 * sum(seq(1,read_len - sum + 1)))
}
double.windows <- double.windows_pre
double.windows[] <- vapply(double.windows,FUN=GetDoubleWindows,numeric(1))
print(double.windows)
diag(double.windows) <- 1/2*diag(double.windows)
print(double.windows)
rownames(double.windows) <- rownames(data)[-nrow(data)]
colnames(double.windows) <- rownames(double.windows)
...
rownames(co.matrix.new) <- rownames(sitesXcounts)[-nrow(sitesXcounts)]
colnames(co.matrix.new) <- rownames(co.matrix.new)
...
GetModelFrequencies <- function(pars, out_model) {
  ...
  raw_probs <<- c(sapply(site.indeces,GetKmerProb,probs_freqs = weights))
  freqs <- raw_probs*site.windows
  names(freqs) <- rownames(sitesXcounts)[-nrow(sitesXcounts)]
  print("freq loop")
  for (i in 1:length(freqs)) {
    print(site_seqs[i])
    ind_longer <- intersect(grep(site_seqs[i], site_seqs),
                              which(nchar(site_seqs) > nchar(site_seqs[i])))
    if (length(ind_longer) > 0) {
      freqs[i] <- freqs[i] - freqs[ind_longer]    
    }
  }
  freqs <<- freqs
  print("site windows")
  print(site.windows)
  print("double occurences")
  print("freqs")
  print(freqs)
  print(double.windows)
  outer_first <- outer(raw_probs, raw_probs,FUN = "*")*double.windows
  rownames(outer_first) <- rownames(double.windows)
  colnames(outer_first) <- rownames(double.windows)
  outer_first <<- outer_first
  print("outer_first")
  print(outer_first)
  overlaps <- GetOverlapProbs(co.matrix.new,weights)
  rownames(overlaps) <- rownames(data)[-nrow(data)]
  colnames(overlaps) <- rownames(overlaps)
  print("overlaps")
  print(round(overlaps, 5))
  coccur <- outer_first + overlaps
  coccur_dif <<- coccur

  print(round(freqs, 5))
  coccur_dif[lower.tri(coccur,diag = TRUE)] <- 0
  print(round(coccur_dif,5))
  print(round(rowSums(coccur_dif)+colSums(coccur_dif),5))
  names(freqs) <- rownames(data)[-nrow(data)]
  rownames(coccur) <- names(freqs)
  colnames(coccur) <- names(freqs)
  print(rowSums(coccur))
  freq_nosite <- 1 - sum(freqs) - sum(coccur[lower.tri(coccur, diag = FALSE)])
  freqs <- freqs - rowSums(coccur) + diag(coccur)
