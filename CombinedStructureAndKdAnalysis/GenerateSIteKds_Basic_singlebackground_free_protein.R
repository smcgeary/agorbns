...
GetModelFrequencies <- function(pars, out_model) {
  ...
  if (tick%%10000 == 0) {
    ...
    MakeSiteIterationPlot(out,paste0(experiment,"_", start, "-", stop, "_", win_left, "-", win_right, "_", sitelist,"_Basic_notfixedproteinonebackground"),mirna, num.kds, num.bgs)  
...
trial <- pars_new$par
    out<<- rbind(out, c(trial, -1*prob_pois))
          out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
                   ...
                   start, "-", stop, "_", win_left, "-", win_right, "_", sitelist, "_Basic_singlebg_free_protein.txt")
