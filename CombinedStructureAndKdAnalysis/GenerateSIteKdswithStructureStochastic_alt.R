...
pars.init <- c(-2*log10(kds.init), -1, -1, -1, -1, -1, 0, 0, 0)
...
GetModelFrequencies <- function(pars_sub,range,pars, c.I.model = c.I.matrix, I.IDs.model = I.IDs) {
  ...
  if (tick%%10 == 0) {
    ...
    mirna,"/", "structure_fit_", start, "-", stop, "_", win_left, "-", win_right, "_stoch_alt.eps"))
...
while (i < 10000) {
  ...
  if (val_new[best] < c(val)) {
    ...
    out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
                     "/equilibrium/kds_with_structure/", k.c.stockago, "_",
                       start, "-", stop, "_", win_left, "-", win_right, "_stoch_alt.txt")
    write.table(file=out_file, out, sep="\t", quote=FALSE, row.names=FALSE,
                col.names=TRUE)

    MakeSiteIterationPlot(out,paste0("structure", start, "-", stop, "_", win_left, "-", win_right,"_stoch_alt"))  
    i <- i + 1
  } else {
    wiggle <- 0.99 * wiggle
    print("stochastic:")
