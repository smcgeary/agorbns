...
GetModelFrequencies <- function(pars_sub,range,pars, c.I.model = c.I.matrix, I.IDs.model = I.IDs) {
  ...
  dev.new(xpos = 20, ypos = 20, height = 12, width = 15)
  par(mfrow=c(2,3))
  # print(-1*prob_pois)
    ...
    for (name in colnames(c.final)) {
      ...
      plot(x_plot, y_plot,col=c(site_cols[IDs_collapse[,1],],"black"),lwd=2,pch=20,xlim=c(0.0000002,.02),ylim=c(0.0000002,.02), log='xy')
      title(main = name)
    }
  return(-1*prob_pois)
}
...
TestModel <- function(pars) {
  model <- SimulateData(pars)[[1]]
  # print(dim(model))
  # print(dim(I_data))
  # print(dim(data))
  # print(dim(I_IDs))
  out <- matrix(sapply(unique(I_data$site), function(site_type) {
                         sapply(seq(1, ncol(model)), function(col_num) {
                           cor(log10(model[which(I_IDs[-nrow(data), 1] == as.character(site_type) & I_data$n > 0 & data[-nrow(data), col_num] > 0), col_num]),
                               log10(data[which(I_IDs[-nrow(data), 1] == as.character(site_type) & I_data$n > 0 & data[-nrow(data), col_num] > 0), col_num]))
                            })
                          }), byrow=TRUE, ncol=5)
  rownames(out) <- unique(I_data$site)
  colnames(out) <- colnames(data)
  return(out)
}


# diagnostic <- TestModel(solution$par)
# par(mfrow = c(1,3))
# plot(colnames(diagnostic), rep(0,5), xlim=c(0.3, 100), ylim=c(-1, 1), log="x", col = "white")
...
#   })
# column_colors <- c("red", "blue", "forestgreen", "purple", "orangered")
...
#   c(diagnostic[row[1], row[2]], sum(data[which(I_IDs[,1] == row[1]), row[2]]), site_cols[row[1],], column_colors[row[2]])
#   })

# plot(x_y_pairs[2, ], x_y_pairs[1, ], col=x_y_pairs[3, ], log='x', cex=2, pch = 19)
# plot(x_y_pairs[2, ], x_y_pairs[1, ], col=x_y_pairs[4, ], log='x', cex=2, pch = 19)

# break
# val <- GetModelFrequencies(trial)

# # print("NOW IT'S OPTIMIZING!!")
# # # Solve the first run of the function, and create the output matrix.
# solution <- optim(trial, GetModelFrequencies,
#                   method="Nelder-Mead",
#                   control=c("maxit"<-20000))
out <- rbind(c(pars.init, 10^10),
             c(pars.init, 10^10))
colnames(out) <- c(rownames(sitesXcounts)[1:num.kds],colnames(data),"k","a","b","-logp")
GetModelFrequencies(0,0,pars.init, c.I.matrix, I.IDs)
break
...
while (converg >= 0.000001) {
  print("hi")
  params_current <- out[nrow(out),]
  for (par_ind in seq(length(params_current) - 1)) {
    # print(colnames(out)[par_ind])
    solution_temp <- optimize(GetModelFrequencies,
                    c((params_current[par_ind] - 5) : (params_current[par_ind] + 5)),
                    range = par_ind,
                    pars = params_current)    
    val <- solution_temp$objective
    print(val)
    params_current[par_ind] <- solution_temp$minimum

  }


  out <- rbind(out,c(params_current,val))
  converg <- CheckMaxDifference(out)
  print(val)

  print(converg)
  # out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
  #                  "/equilibrium/kds_structure/", method, "_", k.c.stockago, "_5-5.txt")
  # write.table(file=out_file, out, sep="\t", quote=FALSE, row.names=FALSE,
  #             col.names=TRUE)

  # # Assign output file For the final parameters and write to it.
  # out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
  #                    "/equilibrium/kds/final_", method, "_", k.c.stockago, "_5-5.txt")
  ...
  # MakeSiteIterationPlot(out,paste0(method,"_5-5"))  
}
