...
GetSitesXcountsCombined <- function(experiment,mirna) {
  sites_file_name <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/",
                       mirna, "/",experiment,"/full_site_count_tables/all_sites_",
                       start, "-", stop,"_", sitelist, "_combinedinput.txt")
  print(sites_file_name)
  sitesXcounts <- read.table(sites_file_name)
  return(sitesXcounts)
}
...
sitesXcounts_combined <- GetSitesXcountsCombined(experiment,mirna)
colnames(sitesXcounts_combined)[3:dim(sitesXcounts_combined)[2]] <- sapply(
  colnames(sitesXcounts_combined)[3:dim(sitesXcounts_combined)[2]], function(x) {
    return(unlist(strsplit(x, split="A", fixed=TRUE))[2])
  }
)
print(sitesXcounts_combined)
seqs <- sitesXcounts_combined[,1]
sitesXcounts_combined <- sitesXcounts_combined[,-1]
sitesXcounts_combined <- sitesXcounts_combined[rowSums(sitesXcounts_combined)>0,]
sitesXcounts_combined <- sitesXcounts_combined[sitesXcounts_combined[,1]>0,]
...
data <- sitesXcounts_combined[,1:7]
...
trial <- pars_new$par
    out<<- rbind(out, c(trial, -1*prob_pois))
          out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
                   ...
                   start, "-", stop, "_", win_left, "-", win_right, "_", sitelist, "_Basic_singlebg_free_protein_combinedinput.txt")
