...
sXc <- lapply(mir_list, function(mirna) {
  out <- lapply(1:5, function(mir_start) {
    ...
    rbind(head(sXc_i, 5), tail(sXc_i, 1))
    # sXc_i
  })
...
OptimizeEquilSitePars <- function(sXc, pars=NULL, combined=TRUE,
  ...
  sXc_mat <- do.call(cbind, sapply(sXc, function(sXc_i) {
    lapply(sXc_i, function(sXc_ij) {
      as.matrix(sXc_ij)
    })
  }))
  ...
  sXc_vec <- as.numeric(as.matrix(sXc_mat))
  # print(data_vec)
  ...
  cost1 <- CostC12mers(initial.pars,
                         dil  = dil,
                         data=data_vec,
                         l = l,
                         L = L_,
                         ...
                         plotname = plotname_)
  # break
  cost2 <- CostCMir7All(initial.pars, sXc_vec, dils=dil, n_x = n_i, combined=FALSE)


  # plot(cost_each_global, cost_each_2_global)
  # abline(0, 1)
  # break
  print(cost1)
  print(cost2)
  ...
  grad_analytical <- GradC12mers(initial.pars,
                                 dil  = dil, data=data_vec,
                    l = l,
                    L = L_,
                    Y = Y,
                    n_i = n_i,
  ...
  grad_analytical2 <- GradCMir7All(initial.pars, sXc_vec, dils=dil, n_x = n_i, combined=FALSE)

  print("done with this")
  grad_numerical <- grad(func=CostC12mers,
                         x=initial.pars,
                         method="simple",
                         dil  = dil,
                         data=data_vec,
  ...
  grad_numerical2 <- grad(CostCMir7All,
                          x=initial.pars,
                          method="simple",
                          sXc=sXc_vec,
                          dils=dil,
  ...
  colors_temp <- c(rep("blue", n_i - 1), "gray",
                   rep("purple", n_i - 1), "gray",
                   rep("green", n_i - 1), "gray",
                   rep("orange", n_i - 1), "gray",
                   rep("red", n_i - 1), "gray",
                   rep("brown", n_mir),
                   rep("black", n_mir))
  ...
  plot(grad_analytical, grad_numerical, col=colors_temp, pch=19)
  abline(0, 1, lty=2)
  plot(grad_analytical2, grad_numerical2, col=colors_temp, pch=19)
  abline(0, 1, lty=2)
  plot(grad_analytical, grad_analytical2, col=colors_temp, pch=19)
  abline(0, 1, lty=2)
  plot(grad_numerical, grad_numerical2, col=colors_temp, pch=19)
  abline(0, 1, lty=2)

  print(cost1)
  print(cost2)

  grad_combined <- cbind(grad_analytical, grad_numerical, grad_analytical - grad_numerical, colors_temp)
  rownames(grad_combined) <- names(initial.pars)
  print(grad_combined)

  print(cbind(grad_analytical, grad_analytical2, grad_analytical - grad_analytical2))
  print(cbind(grad_numerical, grad_numerical2, grad_numerical - grad_numerical2))
