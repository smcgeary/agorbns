...
in_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                   ...
                   "_singlebg_combinedinput_logresidual_PAPER_final.txt")
...
pars_table <- fread(in_file, sep = "\t", header=TRUE, colClasses = "numeric",stringsAsFactors = FALSE)
pars_table <- matrix(unlist(pars_table), nrow=nrow(pars_table), ncol = ncol(pars_table), byrow=FALSE)
pars <- unlist(pars_table[nrow(pars_table),])
pars <- pars[-length(pars)]

pars[which(pars <= -13)] <- pars[which(pars <= -13)] + 3
tick <- 1
print("up to model")

if (sitelist == "12mers") {
  specific_figure_string <- paste0(start, "-", stop, "_", sitelist, "_",
    mirna.start, "-", mirna.stop, "_singlebg_combinedinput_logresidual_PAPER_final2.txt")
} else {
  specific_figure_string <- paste0(start, "-", stop, "_", sitelist, "_",
    "singlebg_combinedinput_logresidual_PAPER_final2.txt")
}
  ...
  if (tick%%1 == 0) {
    ...
    if (sitelist == "12mers") {
      out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                       experiment, "/kds_PAPER/", start, "-", stop, "_", 
                       sitelist, "_", mirna.start, "-", mirna.stop,
                       "_singlebg_combinedinput_logresidual_PAPER_final2.txt")
      PlotSiteKdOptimization(out[,c(seq(1,num.kds-1,length=256),(ncol(out)-3):ncol(out))], specific_figure_string, mirna, 257, num.bgs, colors=colors)
...
colnames(out) <- c(rownames(data),
  ...
  if (sitelist == "12mers") {
    n <- 256
    colors <- c(rainbow_hcl(n, c=50, l=70, start = 0, end = 360*(n-1)/n), "black", "red", "gray")
    out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                       experiment, "/kds_PAPER/", start, "-", stop, "_", 
                       sitelist, "_", mirna.start, "-", mirna.stop,
                       "_singlebg_combinedinput_logresidual_PAPER_final2.txt")
