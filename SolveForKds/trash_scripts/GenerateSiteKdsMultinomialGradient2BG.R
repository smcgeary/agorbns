...
pars.init <- c(Logit(kds.init[-length(kds.init)], 10), -1,-1, 1)

pars.init <- rnorm(length(pars.init), mean=0, sd = 2)
...
names(pars.init) <- c(rownames(data)[-nrow(data)], "bg1","bg2", "AGO")
...
ModelFunction <- function(pars) {
  ...
  # HERE IS THE 2nd BG assignment
  bgs[bg2col] <- 10^pars[num.kds + 2]
  stock.ago <- 10^pars[num.kds + 3]
...
Gradient <- function(pars) {
  ...
  kds  <- c(Logistic(pars[1 : num.kds], 10),1)
  names(kds) <- rownames(data)
  B  <- rep(10^pars[(num.kds + 1)],5)
  B[bg2col] <- 10^pars[num.kds + 2]
  ...
  l.ivec <- c.I.tots
  B.mat <- matrix(B, nrow=nrow(data), ncol = ncol(data), byrow=TRUE)
  ...
  C1.mat <- L - B.mat - 2*A.mat
  C2.mat <- A.mat^2 + (B.mat - L)*A.mat - L*B.mat
  ...
  dc.ai.dF <- R.mat * l.mat * (
    (
      ...
      ((L - A.mat)^2 + K.mat * (4 * B.mat + A.mat)) * f.mat^2
    ) + (
      2 * K.mat * (A.mat * (L - B.mat) + B.mat * (K.mat - 2 * L) - (A.mat + B.mat)^2) * f.mat
    ) + (
      K.mat * (A.mat^3 + 2 * A.mat^2 * (B.mat - L) - B.mat * (B.mat - L) * (K.mat + L) +
               A.mat * (B.mat^2 - 2 * B.mat * K.mat - 3 * B.mat * L + L^2))
    )
  ...
  dc.ai.dA_specific <- R.mat * l.mat * (
    (
      ...
      ((A.mat - L) * C1.mat - 2 * B.mat * K.mat + C2.mat) * f.mat
    ) + (
      - C1.mat * B.mat * K.mat
    )
  ...
  dc.ai.dB <- R.mat * l.mat * (
    (
      ...
      ((L - A.mat) * (A.mat - L) - K.mat * (B.mat + C1.mat)) * f.mat
    ) + (
      + A.mat * B.mat * K.mat - B.mat * K.mat * L - K.mat * C2.mat
    )
  ) * ((K.mat + f.mat) * (C2.mat + C1.mat * f.mat + f.mat^2)^2)^(-1)
  ...
  gradient_with_bg <- log(10)*c(unique(B[-bg2col]),B[bg2col])*c(sum((residuals * dc.ai.dB)[,-bg2col]),sum((residuals*dc.ai.dB)[,bg2col]))
...
solution <- optim(pars,
                  ...
                  gr = Gradient,
                  method = "L-BFGS-B",
                  lower=c(rep(-13, length=num.kds), -2, -2,-1),
                  upper=c(rep(10, length=num.kds), 1, 1, 2),
