...
params <- GetSiteKds(mirna, experiment, n_constant, sitelist)
  ...
  sapply(c("I","I_combined", 40, 12.6, 4, 1.26, 0.4, 0), function(condition) {
    print(condition)
    siteflanksXprob <- data.frame(GetPairingFlankData(mirna, experiment, condition,
                                   n_constant, sitelist, site, mir.start, mir.stop))

  siteflanksXprob$probbin <- cut(siteflanksXprob$plfold^15, prob_limits,labels=prob_strings)
  siteflanksXprob$flankprob <- apply(siteflanksXprob,1,function(row) {
    temp_string <- unlist(strsplit(row[1],split=""))
    flank_l <- paste0(temp_string[1:2],collapse = "")
    flank_r <- paste0(temp_string[3:4],collapse = "")
    flank <- paste0(flank_l, ".",flank_r, collapse = "")
    output <- paste0(flank, "_",row[length(row)], collapse="")
    return(output)
    })

  vector <- aggregate(. ~ flankprob, siteflanksXprob, function(x)
    c(len = length(x)))[,1:2]

  out <- vector[,2]
  names(out) <- vector[,1]
  sapply(names(out), function(name){
    sfXc_prob[name,condition] <<- out[name]
  })
  })
  print(sfXc_prob[1:10,])
  ...
  colnames(sfXc_prob) <- colnames(sitesXcounts)
  sitesXcounts <- rbind(sitesXcounts, sfXc_prob)
  ...
  pch_all <- c(rep(1, num.kds), rep(19, nrow(sfXc_prob)))
  ...
  enrichments.init <- (Norm(c.I.tots)/Norm(rowSums(data)+1))[(nrow(data) - nrow(sfXc_prob) + 1):nrow(data)]
  pars.init <- log10(enrichments.init)
  pars.init <- rep(log10(kd.site),nrow(sfXc_prob))
  names(pars.init) <- rownames(sfXc_prob)
  pars <- pars.init
