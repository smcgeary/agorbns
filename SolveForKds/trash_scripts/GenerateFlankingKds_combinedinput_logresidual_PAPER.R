...
params <- GetKds(mirna, experiment, start, stop, sitelist, log.residual=TRUE, scaled=FALSE)
...
# Assign the background binding and the Ago concentration from the equilibrium
# fitting.
...
print(sitesXcounts)
# Defining the colorspace for the flanking nucleotides, for printing of the
# scatter plot of the fits.
...
specific_figure_string <- paste0(site, "_", start, "-", stop, "_", sitelist, "_",
  "flanking_singlebg_combinedinput_logresidual_PAPER.txt")
...
for (i in seq(1, 2000)) {
  ...
  out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                     ...
                     sitelist, 
                     "_flanking_singlebg_combinedinput_logresidual_PAPER.txt")
