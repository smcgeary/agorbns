  ...
  out <<- rbind(out, c(pars, sumofsquares))
    ...
    if (sitelist %in% c("12mers", "10mers")) {
      out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                       ...
                       "_singlebg_logresiduals_combinedinput_PAPER_final.txt")
...
#                     upper=c(rep(10,length=num.kds),1,1),
#                     control = list(maxit=10, parscale=abs(Gradient(pars))^(0.9)))
...
#                     control = list(maxit=100, parscale=abs(Gradient(pars))/10))
