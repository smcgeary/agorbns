...
in_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                   ...
                   "10mers", "_", mirna.start, "-", mirna.stop,
                   "_singlebg_combinedinput_logresidual_PAPER_final5.txt")
...
pars_10mer <- unlist(pars_table_new[nrow(pars_table_new),])
names(pars_10mer) <- table_names
# print(pars_10mer)
pars.init <- c(sapply(rownames(data)[-nrow(data)], function(name) {
  name.split <-paste0(substr(name,2,11), collapse="")
  return(pars_10mer[name.split])
  }), pars_10mer["bg"], pars_10mer["AGO"])
pars <- pars.init
# print(pars)
tick <- 1
print("up to model")
...
if (sitelist == "12mers") {
  specific_figure_string <- paste0(start, "-", stop, "_", sitelist, "_",
    mirna.start, "-", mirna.stop, "_singlebg_combinedinput_logresidual_PAPER_final5")
} else {
  specific_figure_string <- paste0(start, "-", stop, "_", sitelist, "_",
    "singlebg_combinedinput_logresidual_PAPER_final5")
}
...
print(length(pars))
    ...
    solution <- optim(pars,
                    ...
                    gr = Gradient,
                    method = "L-BFGS-B",
                    lower=c(rep(-20, nrow(data)-1), -2,0),
