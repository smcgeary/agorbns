...
if (sitelist %in% kmer_list_names) {
  ...
  out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                     ...
                     "_singlebg_logresidualsnopseudo_bootstraps_PAPER.txt")

    out_ci90_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                     experiment, "/kds_PAPER/", start, "-", stop, "_", 
                     sitelist, "_", mirna.start, "-", mirna.stop,
                     "_singlebg_logresidualsnopseudo_ci90_PAPER.txt")

    out_ci95_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                     experiment, "/kds_PAPER/", start, "-", stop, "_", 
                     sitelist, "_", mirna.start, "-", mirna.stop,
                     "_singlebg_logresidualsnopseudo_ci95_PAPER.txt")
  ...
  out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                     experiment, "/kds_PAPER/", start, "-", stop, "_", 
                     sitelist, "_singlebg_logresidualsnopseudo_bootstraps_PAPER.txt")

  out_ci90_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                     experiment, "/kds_PAPER/", start, "-", stop, "_", 
                     sitelist, "_singlebg_logresidualsnopseudo_ci90_PAPER.txt")
 
  out_ci95_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                     experiment, "/kds_PAPER/", start, "-", stop, "_", 
                     sitelist, "_singlebg_logresidualsnopseudo_ci95_PAPER.txt")
...
# out_boots <- matrix(, nrow=100, ncol=nrow(sitesXcounts_original)-1 + 3)
# out_names <- c(rownames(sitesXcounts)[-length(rownames(sitesXcounts))],
#                "bg", "AGO", "-logp")


# colnames(out_boots) <- out_names
if (sitelist %in% kmer_list_names) {
  input_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                          experiment, "/kds_PAPER/", start, "-", stop, "_", 
                          sitelist, "_", mirna.start, "-", mirna.stop,
                          "_singlebg_logresidualsnopseudo_PAPER_last.txt")
  } else {
      input_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                          experiment, "/kds_PAPER/", start, "-", stop, "_", 
                          sitelist, 
  ...
  print(input_file)
  pars.base <- fread(input_file,header=TRUE,stringsAsFactors=FALSE)
  names_keep <- colnames(pars.base)
  pars.base <- as.numeric(pars.base)
  names(pars.base) <- names_keep
    print(pars.base)
  ...
  # colnames(pars.base) <- c(rownames(data)[-nrow(data)], "bg", "AGO")
  pars <- c(pars.base)
  pars <- pars[-length(pars)]
  rownames(data)[nrow(data)] <- "None"
  data_inds <- which(rownames(data) %in% c(names(pars.base),"None"))
  data <- data[data_inds,]
  ...
  c.I.tots <- Norm(sitesXcounts_original[data_inds, 1]) * k.c.lib
...
rownames(bounds90) <- c("mean", "lower", "median", "upper")
...
rownames(bounds95) <- c("mean", "lower", "median", "upper")
