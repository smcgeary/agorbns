...
sXc <- SitesAndSingleFlanksXCounts(mirna, site, experiment=experiment,
                                   n_constant=n_constant, sitelist=sitelist)
# Separate site sequences from data file.

pars.matrix <- GetSiteKds(mirna, experiment, n_constant, sitelist)
  tick <- 0
  ...
  pch_all <- c(rep(1, num.kds), rep(19, nrow(sfXc)))
  ...
  pars.init <- rep(log10(kd.site),nrow(sfXc))
