...
def get_site_table(mirna, exp, n_constant, sitelist):
    ...
    print(sites)
    site_seq_map = get_seq_site_map(mirna_trim, sitelist)
    ...
    for cond in conditions:
    	...
    	site_cond_name = ("/lab/solexa_bartel/mcgeary/AgoRBNS/%s"
    					  "/%s/multisite_counts/%s_%s_%s.txt" % (mirna, exp, cond, n_constant, sitelist))
        ...
        for j, multisite in enumerate(multisites):
            ...
            print(ranks)
            print(min(ranks))
            print([k for k in multisite])
            site = [k for k in multisite if order_site_map[k] == min(ranks)][0]
...
def main():
    ...
    mirna, exp, n_constant, sitelist = parse_arguments(arguments)
    siteXcount = get_site_table(mirna, exp, n_constant, sitelist)
    if exp == "equil_seed_nb":
        siteXcount.columns = ["Seq", "I", "A12.6", "A12.6_2", "A4", "A4_2", "A1.26", "A0.4", "A0"]
    else:
        siteXcount.columns=["Seq","I", "I_combined", "A40","A12.65","A4","A1.265","A0.4","A0"]
    site_count_tables_path = get_analysis_path(mirna, exp, "all_sites_%s_%s" %(n_constant, sitelist),"site_count_tables")
    print(siteXcount)
