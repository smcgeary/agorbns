...
if (sitelist == "11mers") {
  specific_figure_string <- paste0(start, "-", stop, "_", sitelist, "_",
    mirna.start, "-", mirna.stop, "_singlebg_combinedinput_logresidual_PAPER_final")
} else {
  specific_figure_string <- paste0(start, "-", stop, "_", sitelist, "_",
    "singlebg_combinedinput_logresidual_PAPER_final")
}
...
ModelLikelihood <- function(pars) {
  ...
  out <<- rbind(out, c(pars, sumoflogsquares))
    ...
    plot(c(lc.final),c(ldata), col = rep(c(rgb(0,0,0,alpha =0.4),rgb(1,0,0,alpha=0.4),rgb(0,1,0,alpha=0.4),
                                      rgb(1,1,0,alpha=0.4), rgb(1,0,1, alpha = 0.4)),each=length(c(lc.final))/5))
...
print(length(pars))
    ...
    solution <- optim(pars,
                    ...
                    gr = Gradient,
                    method = "L-BFGS-B",
                    lower=c(rep(-20, nrow(data)-1), -3,0),
