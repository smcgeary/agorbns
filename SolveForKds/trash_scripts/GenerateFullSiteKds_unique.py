...
def get_site_table(mirna, exp, start, stop, sitelist):
    ...
    for cond in conditions:
    	...
    	site_cond_name = ("/lab/solexa_bartel/mcgeary/AgoRBNS/%s"
    					  "/%s/full_multisite_counts_unique/%s_%s-%s_%s.txt" % (mirna, exp, cond, start, stop, sitelist))
...
def main():
    ...
    site_count_tables_path = get_analysis_path(mirna, exp, "all_sites_%s-%s_%s" %(start, stop, sitelist),"full_site_count_tables_unique")
