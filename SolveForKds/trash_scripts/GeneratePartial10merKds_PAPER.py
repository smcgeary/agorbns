...
def get_site_table(mirna, exp, start, stop, mir_start, mir_stop, kmer_list):
    ...
    for cond in conditions:
    	...
    	site_cond_name = ("/lab/solexa_bartel/mcgeary/AgoRBNS/%s"
    					  "/%s/full_multisite_counts/%s_%s-%s_10mers_%s-%s.txt" % (mirna, exp, cond, start, stop, mir_start, mir_stop))
...
def main():
    ...
    kmer_list = get_kmer_list_constant_insert(mirna, 10, int(mir_start),
                                              int(mir_stop))
    ...
    site_count_tables_path = get_analysis_path(
        mirna, exp, "all_sites_%s-%s_10mers_%s-%s" %(start, stop, mir_start,
                                                     mir_stop),
