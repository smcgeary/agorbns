...
# sfXc <- matrix(rep(s.c/256,256),nrow=nrow(sfXc),ncol=ncol(sfXc),byrow=TRUE)
rownames(sfXc) <- rownames(sfXc_start)
colnames(sfXc) <- colnames(sitesXcounts)
...
B <- bgs
pars.init <- rnorm(nrow(sfXc),mean=kd.site,sd=1)
# pars.init <- Logit(sfXc[,1]/sfXc[,4]*sum(sfXc[,4])/sum(sfXc[,1]),60)
...
ModelFunction <- function(pars) {
  ...
  kds  <- c(Logistic(kds.s,10),1,Logistic(pars,10))
...
ModelLikelihood <- function(pars, print.=FALSE){
  ...
  if (tick %% 10 == 0 & print.==TRUE) {
    ...
    sapply(1:ncol(data.all), function(col) {
      ...
      y_plot <- y_data.all
       points(x_plot, y_plot,col=c(site_cols[rownames(data.all)[1:(length(kds.s)+1)],], colors_flanks), pch=20)
    })
    dev.set(6)
    plot(pars,pars.init,col=colors_flanks)
    print(sumofsquares)
  }
  tick <<- tick + 1
  model <<- model
...
Gradient <- function(pars,print.=FALSE) {
  ...
  grad_derivs <- (10*exp(pars) * (exp(pars) + 1)^(-2) *
                  (colSums(residuals*dc.ai.dF) %*%
                   t(dF.dK.mat[(length(kds.s)+2):num.kds.all,]
                  ) + rowSums((dc.ai.dKj_specific*residuals)[(length(kds.s)+2):num.kds.all,])))
...
out <- matrix(c(pars, ModelLikelihood(pars,print.=TRUE)), nrow=1)
out_names <- c(names(pars), "-logp")
colnames(out) <- out_names

GradientNumerical <- function(pars,print.=FALSE){
  return(grad(ModelLikelihood,pars,method="simple",print.=FALSE))
}
# break
solution <- optim(pars,
                  ModelLikelihood,
