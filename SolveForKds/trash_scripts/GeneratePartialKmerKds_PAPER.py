...
def get_site_table(mirna, exp, start, stop, kmer_length, mir_start, mir_stop):
    ...
    for cond in conditions:
    	...
    	site_cond_name = ("/lab/solexa_bartel/mcgeary/AgoRBNS/%s"
    					  "/%s/full_multisite_counts/%s_%s-%s_%smers_%s-%s.txt" % (mirna, exp, cond, start, stop, kmer_length, mir_start, mir_stop))
...
def main():
    ...
    arguments = ["miRNA", "experiment", "start", "stop", "kmer", "miR_start", "miR_stop"]
    mirna, exp, start, stop, kmer_length, mir_start, mir_stop = parse_arguments(arguments)



    siteXcount = get_site_table(mirna, exp, start, stop, int(kmer_length), int(mir_start),
                                int(mir_stop))
    print(siteXcount)
    if exp == "equil_mmseed_nb":
    ...
    site_count_tables_path = get_analysis_path(
        mirna, exp, "all_sites_%s-%s_%smers_%s-%s" %(start, stop, kmer_length, mir_start,
                                                     mir_stop),
