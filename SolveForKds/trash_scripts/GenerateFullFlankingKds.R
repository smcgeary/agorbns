...
GetSitesXcounts <- function(mirna) {
	sites_file_name <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/",
                       mirna,"/equilibrium/site_count_tables/all_sites_5-5.txt")
...
GetSiteKds <- function(mirna,k.c.stockago) {
	sites_file_name <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna,
	     						 "/equilibrium/kds/final_Nelder-Mead_", k.c.stockago, "_5-5.txt")
...
GetModelFrequencies <- function(par, index=FALSE, pars=FALSE) {
	...
	mirna,"/flanking_fits/model_fits/",site,"_", k.c.stockago, "_5-5.eps"))
		par(par_plots)
...
while (converg >= 0.0001) {
	for (i in 1:length(pars.init)) {
		...
		if (dim(out)[1]%%10 == 1) {
			...
			WriteIterationFile(out[inds,],extension="_5-5")
			WriteFinalParameterFile(out,extension="_5-5")
			MakeIterationPlot(out[inds,],"flanking_fits",extension="_5-5")		
		}
...
while (converg >= .0000001) {
	...
	print(converg)
	WriteIterationFile(out,extension="_5-5")
	WriteFinalParameterFile(out,extension="_5-5")
	MakeIterationPlot(out,"flanking_fits", extension="_5-5")
}
