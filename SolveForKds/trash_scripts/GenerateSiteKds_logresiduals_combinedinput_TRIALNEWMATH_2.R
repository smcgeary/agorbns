...
# print(pars_10mer)
kds.init <- c(r_denomenator + 0.0000001)/c(r_numerator + 0.0000001)
kds.init <- kds.init/max(kds.init)
print(range(kds.init))
pars.init <- rep(1,num.kds+2)
...
if (sitelist %in% c("12mers","10mers")) {
  specific_figure_string <- paste0(start, "-", stop, "_", sitelist, "_",
    mirna.start, "-", mirna.stop, "_singlebg_logresiduals_combinedinput_PAPER_final2")
} else {
  specific_figure_string <- paste0(start, "-", stop, "_", sitelist, "_",
    "singlebg_logresiduals_combinedinput_PAPER_final2")
}
...
c.final <- c.vec_num * c.vec_dem
  ...
  sumofsquares <- sum((log(c.final+1) - log(data+1))^2)
  ...
  out <<- rbind(out, c(pars, sumofsquares))
    ...
    if (sitelist %in% c("12mers", "10mers")) {
      out_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                       ...
                       "_singlebg_logresiduals_combinedinput_PAPER_final2.txt")

      out_last_file <- paste0("/lab/solexa_bartel/mcgeary/AgoRBNS/", mirna, "/",
                 experiment, "/kds_PAPER/", start, "-", stop, "_", 
                 sitelist, "_", mirna.start, "-", mirna.stop,
                 "_singlebg_logresiduals_combinedinput_PAPER_final2_last.txt")
...
c.final <- c.vec_num * c.vec_dem
  ...
  grad_derivs <-  2 * (colSums(residuals*dc.ai.dF) %*% t(dF.dK.mat[-nrow(dF.dK.mat),]) + rowSums(dc.ai.dKj_specific*residuals)[-nrow(dF.dK.mat)])
  ...
  gradient_with_Ago <- 2*sum(residuals * dc.ai.dA)


  gradient_with_bg <- 2*sum(residuals * dc.ai.dB)
