...
params_dropout <- matrix(0,nrow=length(pars),ncol=ncol(data))
print(params_dropout)
...
ModelLikelihood <- function(pars,dropout_col){

  model <- ModelFunction(pars)
  model_norm <- t(t(model) / colSums(model))
  sumofsquares <- -sum(data[,-dropout_col]*log(model_norm[,-dropout_col]))
...
Gradient <- function(pars,dropout_col) {
  # Split up the parameters into the kd and background parameters.
  kds  <- c(Logistic(pars[1 : num.kds], 10),1)
  names(kds) <- rownames(data)
  B  <- 10^pars[(num.kds + 1)]
  ...
  grad_derivs <- (10*exp(pars[1:num.kds]) * (exp(pars[1:num.kds]) + 1)^(-2) *
                  (colSums((residuals*dc.ai.dF)[,-dropout_col]) %*%
                   t(dF.dK.mat[-nrow(dF.dK.mat),-dropout_col]
                  ) + rowSums((dc.ai.dKj_specific*residuals)[,-dropout_col])[-nrow(dF.dK.mat)]))
  ...
  gradient_with_Ago <- log(10)*stock.ago*sum((residuals * dc.ai.dA)[,-dropout_col])

  gradient_with_bg <- log(10)*B*sum((residuals * dc.ai.dB)[,-dropout_col])
...
out <- matrix(c(pars, ModelLikelihood(pars,1)), nrow=1)
