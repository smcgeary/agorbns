...
def main():
    ...
    arguments = ["miRNA", "experiment", "condition", "n_constant", "kmer_len",
                 ...
                 "-weird", "-alt_miRNA", "-jobs", "-temp_path",
                 "-test_binary"]
    args = parse_arguments(arguments)
    (mirna, experiment, condition,
     n_constant, kmer_len, n_ex, uniq,
     buffer3p, final, weird, alt_miRNA,
     jobs, temp_path, test             ) = args
    ...
    if experiment in ["equilibrium_2_tp", "equilibrium_tp"]:
    ...
    if condition == "I_combined":
        ...
        for input_tuple in input_list:
            ...
            # with the `-temp_path` flag and assigna temporary path at that
            # location.
            for i, argument in enumerate(arguments):
            ...
            shell_call = ("bsub -q 18 -n 20 python AssignSiteTypes/"
                          "AssignKmersNew.py %s" %arg_str)
    ...
    if temp_path:
        ...
        kmers_path = get_analysis_path(mirna, experiment, condition,
                                       "kmers_cutoff_resub", ext=extension)
