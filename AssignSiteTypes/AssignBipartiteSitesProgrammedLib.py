...
def assign_site(read_seqs, _mirna, experiment, n_con, n_lib):
        ...
        if name_prog:
            ...
            # 7.iii Check for programmed and threeprime site only.
                ...
                for (l_mirThrP, l_thrP) in l_mirAndThrPs:
                    n_mir = singleThrP_map[name_prog][n_thrp].shape[0]
                    off_l = n_mir + 8
                    off_r = off_l + len_thrp - 1
                    off_mirp = 25 + n_con - len_thrp + 8 + 1
                    ...
                    # 
                    # Define the pairing position.
                    thrp_pos_l = off_l - l_mirThrP
                    name_thrP = "%s-m%s.%s" %(n_thrp, off_l - l_mirThrP, off_r - l_mirThrP)
                    off_mirp_final = off_mirp - l_thrP
                    offset_use = off_mirp_final - thrp_pos_l
                    name_check = "%s|%s|%s" %(name_thrP, off_mirp_final, name_prog)
                    if (11 <= thrp_pos_l <= 13 and
                        4 <= len_thrp <= 9 and
                        0 <= offset_use <= 5):
                        ...
                        # print(len_thrp)
                        # print(thrp_pos_l)
                        # print(off_mirp_final)
                        # print(name_thrP)
                        # print(name_check)
                        # print(_read.seq)
                        # print(' '*l_thrP + "_"*len_thrp + " "*(25 + n_con - l_thrP - len_thrp) + "_"*8)
                        loop_sequence = _read.seq[l_thrP + len_thrp:25 + n_con]
                        # print(" "*(l_thrP + len_thrp) + loop_sequence)
                        # print(offset_use)
                        # print(name_thrP)
                        # print(name_check)
                        loop_sequences[thrp_pos_l][len_thrp][offset_use].append(loop_sequence)
                        # sys.stdout.flush()
    ...
    print(loop_sequences)
    sys.stdout.flush()
    return (progOnly_map, singleSeed_map, singleThrP_map, thrPAndSeed_map, print_counter, loop_sequences)
...
def main():
    ...
    progOnly_map, singleSeed_map, singleThrP_map, thrPAndSeed_map, counter_all, loop_sequences = threads[0]
    ...
    site_counts_path = get_analysis_path(
        mirna, experiment, condition, "site_counts", ext=extension + "_new"
    )
    site_counts_suppcomp_path = get_analysis_path(
        mirna, experiment, condition, "site_counts",
        ext=extension_suppcomp + "_new"
    )
    ...
    # offsets of 0 to +5.
    # loop_sequences[thrp_pos_l][len_thrp][offset_use].append(loop_sequence)
    for thrp_pos_l in range(11, 14):
        for len_thrp in range(4, 10):
            for offset_use in range(0, 6):
                extension = extension_save + "_pos%s_len%s_off%s" %(thrp_pos_l, len_thrp, offset_use)
                if test:
                    extension = "%s_test" %extension
                ...
                loops = []
                for t in threads:
                    loops += t[-1][thrp_pos_l][len_thrp][offset_use]
                print(loops)
