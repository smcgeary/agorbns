...
def get_flanking_nucleotide(read_seqs, order_site_map, start_lim, stop_lim, mirna, experiment):
    ...
    for i in read_seqs:
        ...
        if sites != "None":
            ...
            left_start = start - 2
            right_stop = stop + 2
            if left_start >= 0 and right_stop < 37 + start_lim + stop_lim:
                flank = read_seq[left_start:start]+"."+ read_seq[stop:right_stop]
                if flank == "GC.C":
                    print(read_seq)
                    print(site)
                    print(" "*start + read_seq[start:stop])
                    print(" "*left_start + read_seq[left_start:start]+"_"*(stop - start) + read_seq[stop:right_stop])
...
def main():
    ...
    values = [{kmer: sum([thread[key][kmer]["TGT"] for thread in dict_threads]) for kmer in kmers}
              for key in keys]
    print(values[1:10])
