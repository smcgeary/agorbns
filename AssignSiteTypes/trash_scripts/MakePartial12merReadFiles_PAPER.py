...
def assign_site_type_to_read(read_seqs, kmer_list, start, stop, mir_stop):
    ...
    """
    mir_stop = int(mir_stop)
    constant_seq = kmer_list[0][10 - mir_stop:14 - mir_stop]
    # Sub-function to be used on each site:
    def get_unique_sites(read_seq, kmer_list):
        site_maps = []
        # Iterate over keys in dictionary (site seqs)
        # print(read_seq)
        for index in range(0,len(read_seq)-12+1):
            kmer = read_seq[index:index+12]
            seed = kmer[10 - mir_stop:14 - mir_stop]
            # Check if index is -1
...
def main():
    ...
    (mirna, experiment, condition, start, stop, kmer, mir_start, 
    ...
    extension = "_%s-%s_%smers_%s-%s" %(start, stop, kmer, mir_start, mir_stop)
