...
# FUNCTIONS
def assign_site(read_seqs, _mirna, experiment, n_con, n_lib, split9mers,
                outto11mers, downto2mers, downto3mers, downto4mers, end3prand):
    ...
    for i_r, r in enumerate(read_seqs):
        ...
        if prog_kmer in seqs_supp_comp:
            ...
            # Use all three prime sites longer than 5 nt.
            if (len_match >= kmer_min and len(p_mir_rc) != 0 and
                (len_match < kmer_max or split9mers)):

                # If the site is longer than 9 nt, separate into smaller sites
                # of length 9 nt.
                if len_match > 9 and not outto11mers:
                    p_mir_rc = list(range(p_mir_rc[0], p_mir_rc[0] + len_match - 9 + 1))
                    p_lib = list(range(p_lib[0], p_lib[0] + len_match - 9 + 1))
                    len_match = 9
                    fraction_each_map[5] += 1
...
def main():
    time_start = time.time()
    arguments = [
        "miRNA", "experiment", "condition", "n_constant",  "-split9mers_binary",
        "-outto11mers_binary", "-downto2mers_binary", "-downto3mers_binary",
        "-downto4mers_binary", "-end3prand_binary", "-jobs", "-test_binary"
    ]
    args = parse_arguments(arguments)
    (
        mirna, experiment, condition, n_constant, split9mers, outto11mers,
        downto2mers, downto3mers, downto4mers, end3prand, jobs, test
    ) = args
    ...
    if split9mers:
        extension = "%s_split9mers" %extension
    if outto11mers:
        extension = "%s_outto11mers" %extension
    if downto2mers:
        extension = "%s_downto2mers" %extension
    elif downto3mers:
        extension = "%s_downto3mers" %extension
    elif downto4mers:
        extension = "%s_downto4mers" %extension
    if end3prand:
        extension = "%s_end3prand" %extension
    ...
    if not jobs:
        jobs = 20
    args  = [_mirna, experiment, int(n_constant), rand_length, split9mers,
             outto11mers, downto2mers, downto3mers, downto4mers, end3prand]
    print(args)
    ############################################################################
    threads = []
    ...
    for thread in threads[1:]:
        for prog_key in prog_keys:
            progOnly_map[prog_key] += thread[0][prog_key]
            for seed_key in seed_keys:
                singleSeed_map[prog_key][seed_key] += thread[1][prog_key][seed_key]
                for thrP_key in thrP_keys:
                    thrPAndSeed_map[prog_key][seed_key][thrP_key] += thread[3][prog_key][seed_key][thrP_key]
            for thrL_key in thrL_keys:
                singleThrP_map[prog_key][thrL_key] += thread[2][prog_key][thrL_key]
    ...
    # print(progSite_suppCompSite_map)
    ...
    for prog_key in prog_keys:
        ...
        for seed_key in seed_keys:
            ...
            # progAndSeed_collapsed_map[key_collapsed] = 0
    ...
    print("collapsed 2 length:")
    print(len(progAndSeed_collapsed_map))
    # print(progAndSeed_collapsed_map[key_collapsed])
    # print(pd.DataFrame.from_dict(progAndSeed_suppcomp_map, orient="index"))
    ...
    for prog_key in prog_keys:
        ...
        for thrL_key in thrL_keys:
            ...
            # rownames = ["%s-m%s.%s" %(thrL_key, mir_len - i, mir_len - i + len_thrP - 1)
            #             for i in range(n_mir)]
            mir_len = n_mir + 8
            off_l = mir_len
            off_r = mir_len + len_thrP - 1
            off_mirp = 25 + int(n_constant) - len_thrP + 1 + 8
            names = ["%s-m%s.%s|%s|%s" %(thrL_key,  off_l - i,
                                         off_r - i, off_mirp - j,
                                         prog_key)
                     for i in range(n_mir) for j in range(n_lib)]
            names_collapsed = ["%s-m%s.%s|%s" %(thrL_key, off_l - i,
                                               off_r - i, prog_key)
                               for i in range(n_mir) for j in range(n_lib)]
            names_suppcomp = ["%s-m%s.%s|%s|%s" %(thrL_key, off_l - i,
                                               off_r - i, off_mirp - j, base_key)
                               for i in range(n_mir) for j in range(n_lib)]
            ...
            for name, name_collapsed, name_suppcomp, count in zip(names, names_collapsed, names_suppcomp, array_list):
                progAndThrP_map[name] = count
                # if count != 0 and name_collapsed == "5mer-m10.14|8mer":
                #     print(name_collapsed)
                #     print(count)
                #     print("counts before:")
                #     print(progAndThrP_collapsed_map[name_collapsed])
    ...
    # print(len(progAndThrP_collapsed_map))
    # print(progAndThrP_collapsed_map["5mer-m10.14|8mer"])
    # print(progAndThrP_collapsed_map["5mer-m11.15|8mer"])
    counts = counts.append(
    ...
    print("added df 4")
    print(counts.shape)
    print(counts_collapsed.shape)
    print(counts_suppcomp.shape)
    ...
    site_counts_collapsed_path = get_analysis_path(mirna, experiment, condition,
                                       "programmed_site_counts_collapsed", ext=extension)

    site_counts_suppcomp_path = get_analysis_path(mirna, experiment, condition,
                                       "programmed_site_counts_suppcomp", ext=extension)
