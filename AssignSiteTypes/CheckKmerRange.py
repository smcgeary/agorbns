...
def main():
    ...
    arguments = ["miRNA", "condition", "-n_ex", "-alt_mirna",
                 "-buffer3p_binary" "-weird"]
    (mirna, condition, n_ex, alt_mirna, buffer) = parse_arguments(arguments)
    # Initialize objects:
    _mirnas = [Mirna(mirna)]
    print(_mirnas)
    n_constant = "5"
    ...
    for kmer_len in kmer_range:
        ...
        try: 
            ...
            print('here')
            tops = sorted(_kmerlist_R_I.kmers["TGT"])[::-1]
            inds = np.argsort(-np.array(_kmerlist_R_I.kmers["TGT"]))
            ind_rank = 1
            go = True
            top_kmer = get_kmer_list(kmer_len)[inds[0]]
            top_R = tops[0]
            print(top_kmer)
            while go:
                kmer_2 = get_kmer_list(kmer_len)[inds[ind_rank]]
                if kmer_2[1:] != top_kmer[:-1] and kmer_2[:-1] != top_kmer[1:]:
                    go = False
                    print(kmer_2)
                    R = tops[ind_rank]
                    print(top_R / R)
        ...
        # print(inds)
        # print(inds[0])
        # kmer1 = get_kmer_list(kmer_len)[inds[0]]
        # kmer2 = get_kmer_list(kmer_len)[inds[1]]
        # print(kmer1)
        # print(kmer2)
        # print("%s %s %2.2f" %(kmer1, kmer2, tops[0]/tops[1]))
