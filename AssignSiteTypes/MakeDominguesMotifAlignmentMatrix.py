...
def main():
    ...
    topkmer = topkmers_df.index[0]
    kmers = topkmers_df.index[1:]
    ...
    for key in range(len(output_dict)):
        ...
        while unassigned_weights.iloc[n_col] >= 0.75:
            unassigned_weights = unassigned_weights.iloc[:n_col - 1]
            print(unassigned_weights)
            motif_weight = motif_weight.iloc[:,:n_col - 1]
            print(motif_weight)
            n_col -= 1
    ...
    for i, final_motif in enumerate(final_motif_list):
        ...
        pfm_path = get_analysis_path_burge(rbp, "equilibrium", condition, "pfm",
                                           ext=out_ext + "_%s" %(i + 1))

        print(pfm_path)
        final_motif[1].to_csv(pfm_path, sep="\t", header=False)

    weights_path = get_analysis_path_burge(rbp, "equilibrium", condition, "pfm",
                                           ext=out_ext + "weights")
    ...
    # alignments = []
    # for kmer in kmers:
    #     alignments.append(check_kmer_against_topkmer(kmer, topkmer, kmer_len))

    # print(alignments)

    # return
    # align_nums = [i[0] for i in alignments]

    # align_pos = [i[1] for i in alignments]
    ...
    # kmers_final = [topkmer]
    # print(kmers_final)
    # for i, num in enumerate(align_nums):
    #     if num == 1:
    #         kmer_add = "."*l_ext + kmers[i] + "."*r_ext
    #         kmers_final += [kmer_add]
    #     elif num == 2:
    #         side = align_pos[i]
    #         if side == "l":
    #             kmers_final = ["."*(2 - l_ext) + kmer for kmer in kmers_final]
    #             kmer_add = kmers[i] + "."*(2 + r_ext)
    #             kmers_final += [kmer_add]
    #             l_ext = 2
    #         else:
    #             kmers_final = [kmer + "N"*(2 - r_ext) for kmer in kmers_final]
    #             kmer_add = "N"*(2 + l_ext) + kmers[i]
    #             kmers_final += [kmer_add]
    #             r_ext = 2
        # print("".join([str(i) for i in range(1, 6 + l_ext + r_ext)]))
    print("\n".join(kmers))
    ...
    pfm_path = get_analysis_path_burge(rbp, "equilibrium", condition,
                                            "pfm", ext=out_ext + "_%s" %())


    motif_weight.to_csv(pfm_path, sep="\t", header=False)
