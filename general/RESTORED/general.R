...
PlotStructECDF <- function(mirna, experiment, n_constant, sitelist, site,
  ...
  legend(x=0.225, y=1, cex=0.9, legend=legend.names[conditions], lwd=1, col=exp_cond_colors[conditions],
         bty="n", ncol=1)
}
                                 ...
                                 ypos=20, p_score="plfold")
{
  ...
  lines(x_ecdf,ecdf(data.A[[p_score]]^15)(x_ecdf), col = exp_cond_colors[as.character(condition)],lwd = 2)
  lines(x_ecdf,ecdf(data.I[[p_score]][inds_AU]^15)(x_ecdf), col = exp_cond_colors[as.character(condition)],lty=2)
...
pos=0)
  ...
  legend(x = 1e-8, y = 1, cex = 0.9, legend = c("Input library",
                    ...
                    col = c("black", exp_cond_colors[as.character(condition)],
                                     exp_cond_colors[as.character(condition)]),
                    lty = c(1, 1, 2), bty="n")
  ...
  barplot(effect.df$effect,
          ...
          ylim=c(0, 1),
          col=exp_cond_colors[as.character(effect.df$conditions)],
          border=NA,
          xlab="[AGO2-miR-1] (%v/v)")
