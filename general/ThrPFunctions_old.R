...
MakePairingMatrix_old <- function(
...
# GetAveragePercentileMismatchContribution <- function(
...
FitPairingAndOffsetModel <- function(
  ...
  intercept=FALSE, additive=FALSE, lower_alt=TRUE, log_plus_one=FALSE
) {
  print("in FitPairingAndOffsetModel")
  cutoff <- FALSE
  weights <- FALSE
  ...
  if (!use_global_df) {
    # df_all <<- do.call(
    #   "rbind",
    #   lapply(offsetmin:offsetmax, MakeNucleotideContributionDf, mirna=mirna,
    #          experiment=experiment, n_constant=n_constant, sitelist=sitelist,
    #          corrected_kds=corrected_kds, collapsemm=collapsemm,
    #          len_min=len_min, len_max=len_max, pos_3p_min=pos_3p_min,
    #          pos_3p_max=pos_3p_max, supp_base=supp_base,
    #          offset_base=offset_base, site_base=site_base, loop=loop,
    #          cutoff=cutoff, kd_fc=kd_fc)
    # )
  ...
  if (log_plus_one) {
    Model <- function(pars) {
      ...
      return(log(exp(pars$pairing[df_all$pairing] + pars$offset[df_all$offset]) + 1))
    }
    dModel_dPars <- function(pars) {
      ...
      K_base <- exp(val.pairing + val.offset)
      ...
      dM.dp_pairing <- sapply(1:n_pairing, function(ind) {
        ...
        out[inds_par] <- (K_base/(K_base + 1)*sqrt(df_all$weight))[inds_par]
        out
      })
    ...
    Model <- function(pars) {
      ...
      # # print(pars$offset)
      # print("___________")
      # print(head(df_all$pairing))
      # print(head(df_all$offset))
      val_pairing <- pars$pairing[df_all$pairing]
      val_offset <- pars$offset[df_all$offset]
      # print(head(val_pairing))
      # print(head(val_offset))
    ...
    dModel_dPars <- function(pars) {
      ...
      if (exponential) {
        ...
        #   row * c(pars$pairing, pars$offset[offset_names_use])
        # }))
        # vec_use <- c(pars$pairing, pars$offset[offset_names_use])
        out <- t(t(out)*c(pars$pairing, pars$offset[offset_names_use]))
        # out_current <<- out
        # out_alt <<- out_pre %*% c(pars$pairing, pars$offset[offset_names_use])
      }
      if (intercept) {
        out <- cbind(out, rep(1, nrow(df_all)))
        colnames(out)[ncol(out)] <- "b"
  ...
  LossFunction <- function(
    ...
    SSE <- sum(df_all$weight*(df_all$logkd - model_vals)^2)
    if (tick %% 10 == 0) {
      plot(df_all$logkd, model_vals)
    }
    tick <<- tick + 1
    return(SSE)
  }
...
Gradient <- function(
    ...
    # model <- val.pairing*val.offset + pars$b
    ...
    #   return(sum((residuals*val.offset)[inds_sum]))
    # })
    # # Calculate the derivative of each of the offset coefficients.
    # offset_names_use <- setdiff(names(pars$offset), as.character(fixed_offset))
    # dSSE.dpars_offset <- sapply(offset_names_use, function(name) {
    #   inds_sum <- which(df_all$offset == name)
    #   return(sum((residuals*val.pairing)[inds_sum]))
    # })
    # # Concatenate the two components to make the entire gradient vector.
    # dSSE.dpars <- c(dSSE.dpars_pairing, dSSE.dpars_offset)
    # if (length(inds_zero_grad) != 0) {
    #   dSSE.dpars[inds_zero_grad] <- 0
    # }
    # names(dSSE.dpars) <- c(names(pars$pairing), offset_names_use)
    # if (exponential) {
    #   dSSE.dpars <- dSSE.dpars * c(pars$pairing, pars$offset[offset_names_use])
    # }
    # if (intercept) {
    #   dSSE.dpars <- c(dSSE.dpars, sum(residuals))
    #   names(dSSE.dpars)[length(dSSE.dpars)] <- "b"
    # }
    # # print(tick)
    # # if (tick %% 1 == 0) {
...
FitPairingAndOffsetModelForAllOffsets <- function(
  ...
  exponential=FALSE, intercept=FALSE, additive=FALSE, lower_alt=TRUE
) {
...
FitPairingAndOffsetModelNew <- function(
  mirna, experiment, n_constant=3, sitelist="progthrp_suppcomp",
  corrected_kds=TRUE, sumseed=FALSE, offset_lim=c(-4, 16), len_lim=c(4, 11),
  pos_lim=c(9, 23), F_method=FALSE, F_exact=TRUE, use_global_df=FALSE,
  supp_base=FALSE, offset_base=FALSE, site_base=NULL,
  loop=FALSE, cutoff=FALSE, kd_fc=TRUE
) {
  print("in FitPairingAndOffsetModel")
  if (!use_global_df) {
    df_all <<- SubfunctionCall(MakeKdDataFrame)
  ...
  # Assign numbers of pairing and offset parameters.
  n_pairing <- length(unique(df_all$pairing))
  n_offset <- length(unique(df_all$offset))
  # Subfunctions required to fit the model:
  ...
  AssignPars <- function(pars) {
    ...
    # and gradient functions.
    pars.pairing <- pars[1:n_pairing]
    pars.offset <- pars[(n_pairing + 1):(n_pairing + n_offset)]
    # offset_vals <- as.numeric(names(pars.offset_all))
    # pars.offset_min <- pars.offset_all[which(offset_vals < fixed_offset)]
    # pars.offset_max <- pars.offset_all[which(offset_vals > fixed_offset)]
    # pars.offset_fixed <- c(1)
    # names(pars.offset_fixed) <- as.character(fixed_offset)
    # pars.offset <- c(pars.offset_min, pars.offset_fixed, pars.offset_max)
    return(list(pairing=pars.pairing, offset=pars.offset)) 
  }
  ...
  dModel_dPars <- function(pars) {
    ...
    dM.dp_pairing <- sapply(1:n_pairing, function(ind) {
      inds_par <- which(df_all$pairing == names(pars$pairing)[ind])
      out[inds_par] <- (K_base/(K_base + 1))[inds_par]
      out
    })
    # Calculate the derivative of each of the offset coefficients.
    # offset_names_use <- setdiff(names(pars$offset), as.character(fixed_offset))
    dM.dp_offset <- sapply(names(pars$offset), function(name) {
      inds_par <- which(df_all$offset == name)
      out[inds_par] <- (K_base/(K_base + 1))[inds_par]
      out
    })
    # Calculate the derivative of the base coefficient.
    out <- cbind(dM.dp_pairing, dM.dp_offset)
    colnames(out) <- c(names(pars$pairing), names(pars$offset))
    dM_dp_global <<- out
    return(out)
  }
  ...
  # method that doesn't explicitly iterate over all the offset values, so they
  # are now out of order.
  offset_names <- sort(unique(as.integer(df_all$offset)))
  print(n_pairing)
  print(n_offset)
  print(length(pairing_names))
  print(length(offset_names))
  ...
  opt <- optim(pars.init, LossFunction, gr=Gradient,  method="L-BFGS-B",
               lower=rep(lower_use, length(pars.init)),
               upper=rep(20, length(pars.init)), control=list(maxit=1e7))
  coefs <- opt$par

  SSE_global <<- opt$val
  model_values <- Model(opt$par)

  print(cor(model_values, df_all$logkd)^2)
  ############### Get the error for all but fixed offset ##################
  # Define degrees of freedom for error estimation:
  n <- nrow(df_all)
  p <- length(coefs)
...
FitPairingOffsetAndMismatchModelSingleOld <- function(
...
FitPairingOffsetAndMismatchModelWithError_old <- function(
  ...
  offset_base=FALSE, site_base=NULL, loop=FALSE, cutoff=FALSE,
  exponential=FALSE, intercept=FALSE, fullpairs=FALSE
) {
  ############ Solve the first offset and allocate the dataframes ##############
  ...
  if (fullpairs) {
    ...
    mm_i <- 1
    for (offset_i in ((offset_lim[1] + 2):offset_lim[2])) {
      coefs_new <- SubfunctionCall(FitPairingOffsetAndMismatchModelSingle, use_global_df=TRUE,
                                 fixed_offset=offset_i, fixed_mm=mm_i)$coefs
      # rows_use <- setdiff(rownames(coefs_new), as.character(offset_i))
  ...
  # dev.new(xpos=20, ypos=20, height=10, width=5)
  # image(lowerCI_mat)
  # dev.new(xpos=520, ypos=20, height=10, width=5)
  # image(upperCI_mat)
  # Assign the average of each of the UpperCI and LowerCI matrices to the
  # appropriate columns of the starting offset dataframe, and return this
  # object.
...
SwapProgrammedKds_old <- function(kds, prog_mm=TRUE, suppcomp=FALSE, sumseed=FALSE) {
...
GetDeltaDeltaGResidual_old <- function(
  mirna, experiment, start_mm, stop_mm, n_constant=3, bulge=FALSE,
...
MakePositionalMismatchDf_old <- function(
  mirna, experiment, len, n_constant=3, bulge=FALSE, model_values=FALSE,
  mm_and_bulge=TRUE, new=TRUE, kd_fc=TRUE, win_average=3, corrected_kds=TRUE,
  frac_dG=FALSE
) {
...
MakeAllLibraryMismatches_old <- function(
...
GetBulgeAndMismatchValues_old <- function(
...
GetAllProgrammedBulgeAndMismatchDifferences_old <- function(
...
MakeBeckerThreePDataFrame_old <- function(
...
MakeBeckerPairingMatrix_old <- function(
...
# GetMeanEffectSizesAndPValues <- function(
#   mirna="let-7", exp_type="parallel", experiment="twist_reporter_assay_3p_2_tp",
#   add_bulges=FALSE, aggregate=TRUE, dual_site=FALSE
# ) {
#   if (mirna == "let-7") {
...
#   output_df <-  data.frame(config=c(), site=c(), rep=c(), p=c())
...
#     se_difference <- sd(y)/sqrt(length(y))
#     t_statistic <- mean(y)/se_difference
#     p_val <- pt(t_statistic, df=length(y) - 1)
#     output_df <- rbind(
#       output_df,
#       data.frame(config=config_str, site=base_site, rep=mean(y), p=p_val)
#     )
#   }
...
#       output_df <- rbind(
#         output_df,
#         data.frame(config=config_str, site=site_label, rep=mean(y), p=p_val)
#       )
#     }
#   }
#   if (dual_site) {
...
#     output_df <- rbind(
#       output_df,
#       data.frame(config=config_str, site="All_lin-41", rep=mean(y), p=p_val)
#     )

#     ind_lin_41 <- which(log2fc_df[, "Seed"] == "lin-41_UTR")
#     log2fc_df_use <- log2fc_df[ind_lin_41, 5:ncol(log2fc_df)]
#     y <- unlist(log2fc_df_use) - bg_subtract
#     se_difference <- sd(y)/sqrt(length(y))
#     t_statistic <- mean(y)/se_difference
#     p_val <- pt(t_statistic, df=length(y) - 1)
#     output_df <- rbind(
#       output_df,
#       data.frame(config=config_str, site="lin-41_UTR", rep=mean(y), p=p_val)
#     )
#   }
#   return(output_df)
# }

# GetPairedEffectSizesAndPValues <- function(
#   mirna="let-7", exp_type="parallel", experiment="twist_reporter_assay_3p_2_tp",
#   add_bulges=FALSE, aggregate=TRUE, dual_site=FALSE
# ) {
#   if (mirna == "let-7") {
...
#   # output_df <-  data.frame(config=c(), site=c())
#   output_df <- matrix(nrow=0, ncol=2 + width)
#   print(dim(output_df))
#   if (dual_site)  config_str <- "dual"
#   else            config_str <- "single"
...
#       output_df <- rbind(
#         output_df,
#         t(data.frame(c(config=config_str, site=site_label, y)))
#       )
#     }
#   }
#   # rownames(output_df) <- 1:nrow(output_df)
#   # # Workaround due to not being able to pre-define the matrix.
#   # output_df <- as.data.frame(output_df)
#   # output_df[, 3:ncol(output_df)] <- lapply(output_df[, 3:ncol(output_df)], as.character)
#   # output_df[, 3:ncol(output_df)] <- lapply(output_df[, 3:ncol(output_df)], as.numeric)
#   if (dual_site) {
#     inds <- which(log2fc_df[, "Seed"] == "lin-41")
#     log2fc_df_use <- log2fc_df[inds, 5:ncol(log2fc_df)]
#     y <- unlist(log2fc_df_use) - bg_subtract
...
#     output_df <- rbind(
#       output_df,
#       t(data.frame(c(config=config_str, site="All_lin-41", y)))
#     )
...
#     output_df <- rbind(
#       output_df,
#       t(data.frame(c(config=config_str, site="lin-41_UTR", y)))
#     )
#   }
#   rownames(output_df) <- 1:nrow(output_df)
#   output_df <- as.data.frame(output_df)
#   output_df[, 3:ncol(output_df)] <- lapply(output_df[, 3:ncol(output_df)], as.character)
#   output_df[, 3:ncol(output_df)] <- lapply(output_df[, 3:ncol(output_df)], as.numeric)
#   output_df[, 1:2] <- lapply(output_df[, 1:2], as.character)
#   print(output_df)
#   output_2_df <-  data.frame(config=c(), site=c(), rep=c(), p=c())


#   if (dual_site) {
#     inds_compare <- c("9mer-m11.19-A", "9mer-m11.19-T", "9mer-m11.19-AAAA")
#     print(output_df)
#     mean_vals <- colMeans(output_df[which(output_df$site %in% inds_compare), 3:ncol(output_df)])
#     output_df <- rbind(output_df,
#       c(config=config_str, site="9mer-m11.19-A/T/AAAA", mean_vals)
#     )
#   }
#   output_df[, 3:ncol(output_df)] <- lapply(output_df[, 3:ncol(output_df)], as.character)
#   output_df[, 3:ncol(output_df)] <- lapply(output_df[, 3:ncol(output_df)], as.numeric)
...
#   if (!dual_site) {
#     comparisons <- matrix(c("9mer-m11.19-AAAA", "9mer-m11.19-A",
#                             "9mer-m11.19-AAAA", "9mer-m11.19-T",
#                             "9mer-m11.19-A", "9mer-m11.19-none",
#                             "9mer-m11.19-T", "9mer-m11.19-none",
#                             "9mer-m11.19-TTTT", "9mer-m11.19-none",
#                             "9mer-m13.21-TTTT", "9mer-m13.21-AAAA",
#                             "4mer-m13.16-TTTT", "4mer-m13.16-AAAA"), byrow=TRUE, ncol=2)
#   } else {
#     comparisons <- matrix(c("All_lin-41", "9mer-m11.19-A/T/AAAA",
#                             "lin-41_UTR", "9mer-m11.19-A/T/AAAA"), byrow=TRUE, ncol=2)
#   }
#   print(comparisons)
#   print(output_df)
#   apply(comparisons, 1, function(row) {
#     print(row)
#     df_t_test <- output_df[which(output_df$site %in% row), 3:ncol(output_df)]
#     print(df_t_test)
#     site_label <- sprintf("%s|%s", row[1], row[2])
#     print(site_label)
#     print(df_t_test[2, ])
#     print(df_t_test[1, ])
#     print(unlist(df_t_test[2, ]) - unlist(df_t_test[1, ]))
#     y <- unlist(df_t_test[2, ] - df_t_test[1, ])
#     print(y)
#     se_difference <- sd(y)/sqrt(length(y))
#     t_statistic <- mean(y)/se_difference
#     p_val <- 2*pt(-abs(t_statistic), df=length(y) - 1)
#     output_2_df <<- rbind(
#       output_2_df,
#       data.frame(config=config_str, site=site_label, rep=mean(y), p=p_val)
#     )
